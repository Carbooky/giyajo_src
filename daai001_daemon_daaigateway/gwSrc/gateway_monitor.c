...
int gw_network_status_temporary=NETWORK_LINK_UNKNOWN;
...
void *thread_network_link_check(void *argu){
	...
	while(1){
		...
		gw_network_status_temporary = NETWORK_LINK_LOST;
		gw_network_status_temporary = network_link_check();
		puts("Update network status !");
		sleep(NET_PROB_SLEEP_SEC);
	
	
	//sprintf(str_2, "ck tns=%d,ct=%d", gw_network_status_temporary,cnt);
...
int lost_cnt=0;
int gw_global_network_status = NETWORK_LINK_LOST;
void network_link_check_nonblocking(void){
	if(gw_network_status_temporary == NETWORK_LINK_LOST){
		if(lost_cnt > 2)
			gw_global_network_status = NETWORK_LINK_LOST;
		else
			lost_cnt ++;
	}else{
		lost_cnt = 0;
		gw_global_network_status = gw_network_status_temporary;
	}
	
}

void kill_ping_n_ntpdate_ps(void){
	char dummy_rsp[128];

	// kill ping 
	syscall_getRsp("killall -9 ping", dummy_rsp);

	syscall_getRsp("killall -9 ntpdate", dummy_rsp);

	printf("force kill ping & ntpdate process !!\n");
}
...
**********************/
		...
		network_status = gw_global_network_status;

		//sprintf(str_1, "nb ns=%d rt=%d", network_status,total_reconnect_retry_cnt);
...
**********************/	
	// network
		...
		if(network_status==NETWORK_LINK_LOST){
			...
			if(total_reconnect_retry_cnt < 8){
				...
				kill_ping_n_ntpdate_ps();	// suppose to kill 1st of ping in gateway_monitor
				kill_ping_n_ntpdate_ps();   // suppose to kill 2nd of ping in daai_daemon
			...
			} else if (total_reconnect_retry_cnt < 9){
				...
				// 1.2. do DHCP connect
   				...
   				kill_ping_n_ntpdate_ps();
   				kill_ping_n_ntpdate_ps();

			} else if(total_reconnect_retry_cnt < 10){				
				// Wifi
				...
				if(total_reconnect_retry_cnt >= 10)
					total_reconnect_retry_cnt = 0;
			}
			printf(" #%d/10 Network re-connect times !!\n", total_reconnect_retry_cnt);
		}
